...
def main():
    ...
    while True:
        status = input(f"Введите статус, по которому необходимо "
                       f"выполнить фильтрацию. Доступные для фильтровки "
                       f"статусы: {', '.join(available_statuses)}: ").upper()
    ...
    # Сортировка по дате
    sort_by_date = input("Отсортировать операции по дате? "
                         "Да/Нет: ").lower()
    if sort_by_date == "да":
        ...
        if sort_order == "по возрастанию":
            ...
            transactions.sort(key=lambda x: datetime.strptime(x.get("date"),
                                                              "%Y-%m-%dT%H:%M:%S.%f"),
                              reverse=True)
        else:
    ...
    # Фильтрация по рублю
    filter_by_rub = input("Выводить только рублевые транзакции? "
                          "Да/Нет: ").lower()
    if filter_by_rub == "да":
        transactions = [t for t in transactions if t.get(
            "operationAmount", {}).get("currency", {}).get("name") == "RUB"]

    # Фильтрация по слову в описании
    filter_by_word = input("Отфильтровать список транзакций по "
    ...
    if not transactions:
        ...
        logger.info("Не найдено ни одной транзакции, "
        ...
        for i, transaction in enumerate(transactions):
            ...
            if account:
                ...
                print(f"{datetime.strptime(
                    date, '%Y-%m-%dT%H:%M:%S.%f'
                ).strftime('%d.%m.%Y')} {description}\nСчет "
